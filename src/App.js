...
function App() {
  ...
  const [layers, setLayers] = useState([]); // This will now be updated from Sidebar
  const [target, setTarget] = useState(null);

  const textRef = useRef(null);
  const coverRef = useRef(null);
  const layerRefs = useRef([]); // Store refs for additional text layers
  ...
  const handleLayersChange = (newLayers) => {
    setLayers(newLayers);
    // Update refs array to match layers length
          ...
          {canvasUrl ? (
            ...
            >
              ...
              {layers.map((text, index) => {
                if (!text.trim()) return null; // Don't render empty text layers
              ...
              {target && (
                <Moveable
                  ...
                  onRenderStart={(e) =>
                    (e.target.style.outline = "2px solid #00ff00")
                  }
